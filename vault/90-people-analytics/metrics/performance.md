...
| **9-box talent grid**<br><sub>Diagnostic</sub> | Distribution of the rated active population across the performance x potential 9-box grid. | `count(active rated employees) by (performance_band x potential_band)`<br>_protocol:_ Map the rating scale to three performance bands and pair with the potential rating; use only for development/succession conversations, never as an automated action trigger. | employee_snapshot | Succession, development, and calibration discussions (never pay or termination). | ✓ calculate, trend, segment, draft_summary, surface_to_human<br>✗ change_rating, recommend_termination, make_hiring_decision |
