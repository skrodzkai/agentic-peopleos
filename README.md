...
the spine, and what each control addresses. Or run the whole governed loop locally:
...
- **Evidence Graph v1** ([`core/evidence.py`](core/evidence.py),
  ...
  action consumes that authorization once; substituted artifacts and approval reuse fail ledger validation.
  Production executors must run that validation transactionally before the external side effect.
