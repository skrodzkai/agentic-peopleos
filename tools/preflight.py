...
def main():
    ...
    # 1c. Reconcile the deletion-resistant list above to the artifact catalog discovered from
    # SPECS/reference verticals.  A new, removed, renamed, or uncatalogued dashboard cannot hide
    # behind a manually adjusted count.
    try:
        inventory = evidence_portfolio.portfolio_inventory(REPO)
        enumerated = set(REQUIRED_OUTPUTS)
        for agent_id, report, digest in inventory:
            for path in (report, digest, evidence_portfolio.sidecar_path(report),
                         evidence_portfolio.sidecar_path(digest)):
                rel = str(path.relative_to(REPO))
                if rel not in enumerated:
                    errors.append(f"managed artifact is not deletion-resistant in preflight: {rel}")
    except evidence_portfolio.PortfolioEvidenceError as exc:
        errors.append(f"managed artifact inventory does not reconcile: {exc}")
