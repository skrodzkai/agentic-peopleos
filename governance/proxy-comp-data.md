...
- **GitLab (GTLB) CFO — the ~$0.19M stub is EXCLUDED from the distribution.** GitLab's FY2026 SCT lists three CFO rows: an incoming CFO appointed near fiscal year-end (~$186K, $0 LTI — a stub, not a representative annual figure), a Former Interim CFO, and the **full-year Former CFO (~$5.99M)**. The incoming stub is marked `EXCLUDE:` in `proxy_comp.csv` and dropped from the percentile math (it would pull the CFO low tail toward zero); the engine then retains the **full-year Former CFO** as GitLab's CFO observation, and the dashboard/digest note the excluded transition row as a count.
- **BILL CFO ~$13.9M** — a large one-time equity grant; as-disclosed. (BILL's principal financial officer for FY2025 is titled *President & COO* in the SCT — a mid-year title change; he was the PFO for the full disclosed year, so the CFO bucket is correct.)
