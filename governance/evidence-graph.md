...
| `check` | The contract/reconciliation that ran, its implementation, and pass/fail/not-run state. |
...
For a `publish.*` workflow, ledger schema 1.1 requires that envelope on the recommendation, approval,
and action. Replay verifies exact equality at both boundaries. The first fully valid action consumes the
