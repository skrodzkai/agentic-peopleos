...
lf_entry = bundle.artifact_entry("line one\nline two\n", report_manifest)
lf_bytes_entry = bundle.artifact_entry(b"line one\nline two\n", report_manifest)
crlf_entry = bundle.artifact_entry(b"line one\r\nline two\r\n", report_manifest)
ok(lf_entry["content_hash"] == lf_bytes_entry["content_hash"],
   "string and exact UTF-8 bytes produce the same LF artifact hash")
ok(lf_entry["content_hash"] != crlf_entry["content_hash"],
   "newline translation changes the authorization target instead of passing silently")

changed_graph = copy.deepcopy(report_manifest)
changed_graph["claims"][0]["statement"] = "The answer remains 42, with revised wording."
...
bad_bundle = copy.deepcopy(evidence_bundle)
bad_bundle["artifacts"][0]["material_claim_ids"] = ["claim.valid", 7]
ok(any("sorted unique stable ids" in issue for issue in bundle.validate_bundle(bad_bundle)),
   "mixed-type material claim ids fail validation without a sorting exception")
...
for schema, definition, valid in (
        (bundle_schema, "id", "bundle.valid"),
        (bundle_schema, "date", "2026-01-31"),
        (bundle_schema, "hash", "sha256:" + ("a" * 64)),
        (auth_schema, "id", "bundle.valid"),
        (auth_schema, "hash", "sha256:" + ("a" * 64))):
    ok(bool(json_schema.validate(valid + "\n", schema["$defs"][definition])),
       "published %s schema rejects a trailing newline" % definition)
...
other_bundle = bundle.build_bundle("bundle.test.february", [
    ("<html><body>42</body></html>", report_manifest),
    ("# Digest\n42\n", digest_manifest),
])
other_auth = bundle.authorization_envelope(other_bundle)
multi_cycle = path.with_name("multi-cycle.events.jsonl")
rows = []
for authorization in (auth, other_auth):
    rows.extend(json.dumps({"type": event_type, "authorization": authorization},
                           sort_keys=True, separators=(",", ":"))
                for event_type in ("recommendation", "approval", "action"))
multi_cycle.write_bytes(("\n".join(rows) + "\n").encode("utf-8"))
multi_cli = subprocess.run([sys.executable, "-m", "core.evidence_bundle", "validate", str(path),
                            "--ledger", str(multi_cycle)], cwd=repo, capture_output=True, text=True)
ok(multi_cli.returncode == 0 and "3 ledger authorization event(s) match" in multi_cli.stdout,
   "bundle CLI selects its cycle from a multi-bundle ledger")
...
invalid_bundle_path = path.with_name("invalid-bundle.json")
invalid_bundle_path.write_text('{"schema_version":"1.0"}\n', encoding="utf-8")
invalid_cli = subprocess.run([sys.executable, "-m", "core.evidence_bundle", "validate",
                              str(invalid_bundle_path), "--ledger", str(ledger)], cwd=repo,
                             capture_output=True, text=True)
ok(invalid_cli.returncode == 1 and "Traceback" not in invalid_cli.stderr,
   "ledger validation reports an invalid bundle without indexing missing fields")
