...
def artifact_entry(content, manifest, content_uri=None, evidence_uri=None):
    ...
    if not isinstance(content, str):
        raise EvidenceBundleError("rendered artifact content must be text")
    ...
    return {
        ...
        "content_hash": ev.hash_text(content),
...
def validate_bundle(bundle):
    ...
    for index, artifact in enumerate(artifacts):
        ...
        elif claim_ids != sorted(set(claim_ids)) or any(not _valid_id(cid) for cid in claim_ids):
...
def build_bundle(bundle_id, rendered_manifests, artifact_uris=None):
    ...
    unknown_locations = sorted(set(artifact_uris) - artifact_ids)
...
def _ledger_authorizations(path):
    authorizations = []
    for line_number, line in enumerate(Path(path).read_text(encoding="utf-8").splitlines(), start=1):
        ...
        if event.get("type") in ("recommendation", "approval", "action") and \
                event.get("authorization") is not None:
            authorizations.append((line_number, event["authorization"]))
    if not authorizations:
        raise EvidenceBundleError("ledger contains no recommendation/approval/action authorization")
...
def main(argv=None):
    ...
    try:
        ...
        if args.ledger:
            ledger_authorizations = _ledger_authorizations(args.ledger)
            for line_number, authorization in ledger_authorizations:
