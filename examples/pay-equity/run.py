...
def render_digest(report):
    ...
    if eu["potential_joint_assessment"]:
        ...
        lines.append(f"- **EU 5% screen flags** {eu['n_flagged']} category ({flagged}) — **Indicated potential "
                     f"joint-pay-assessment obligation**, pending objective-factor justification / legal "
                     f"review (Article 10 allows six months).")
    else:
        lines.append("- **None: no EU 5% category trigger** on the mean.")
